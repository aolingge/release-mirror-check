...
function usage() {
  console.log(`release-mirror-check v${VERSION}

Usage:
  release-mirror-check --path FILE_OR_DIR
  release-mirror-check FILE_OR_DIR --markdown
  release-mirror-check FILE_OR_DIR --sarif > results.sarif
  release-mirror-check FILE_OR_DIR --annotations
  release-mirror-check FILE_OR_DIR --redact
...
try {
  ...
  console.error(`release-mirror-check: ${error.message}`);
  process.exit(2);
}
